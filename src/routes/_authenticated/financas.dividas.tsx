...
import { Plus, Trash2, Check, RotateCcw, ArrowDownLeft, ArrowUpRight } from "lucide-react";
...
function DividasPage() {
  ...
  const { devo, devem, list } = useMemo(() => {
    ...
    return { devo, devem, list };
  }, [debts, filter]);

  const saldo = devem - devo;
  ...
  const openEdit = (d: FinDebt) => { setEditing(d); setDialogOpen(true); };
  ...
  return (
    <main className="px-5 pt-2 pb-6 space-y-4">
      ...
      <Card className="p-5 bg-gradient-to-br from-accent to-surface border-accent/50">
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          <div className="bg-surface/60 rounded-lg p-3 border border-border">
            ...
            <p className="font-mono text-lg mt-1 text-primary privacy-blur">{fmtEUR(devem)}</p>
          </div>
          <div className="bg-surface/60 rounded-lg p-3 border border-border">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground flex items-center gap-1">
              <ArrowUpRight className="w-3 h-3 text-destructive" /> Eu devo
            </p>
            <p className="font-mono text-lg mt-1 text-destructive privacy-blur">{fmtEUR(devo)}</p>
          </div>
        </div>
      </Card>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button size="icon" onClick={openNew} className="h-10 w-10 shrink-0 rounded-xl">
              ...
              <Card key={d.id} className="p-3.5 bg-surface border-border">
                <div className="flex items-center gap-3">
                  <button onClick={() => handleToggle(d)}
                    ...
                    aria-label={d.pago ? "Marcar como pendente" : "Marcar como pago"}>
